"""
Consejero del Rey — script de consulta.
Toma una situación escrita, busca los fragmentos más relevantes entre
los 14 libros cargados, y genera una respuesta de "consejero" usando Claude.

Uso:
    python consultar.py
    (te pide la situación de forma interactiva)

    o bien:
    python consultar.py --situacion "texto de tu situación aquí"
"""
...
TOP_K = 12  # cantidad de chunks a recuperar para darle contexto al consejero
...
def consultar(situation):
    print("\nBuscando fragmentos relevantes en los 14 libros...")
    query_embedding = embed_query(situation)
    chunks = search_relevant_chunks(query_embedding)

    print(f"Encontrados {len(chunks)} fragmentos relevantes. Consultando al consejero...\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--situacion", help="Tu situación. Si no se pasa, se pide interactivamente.")
    args = parser.parse_args()

    situation = args.situacion
    if not situation:
        print("Contame tu situación (Enter dos veces para terminar):")
        lines = []
        while True:
            line = input()
